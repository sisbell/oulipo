...
/**
 * Keystore for importing keys from JSON format and for exporting keys to JSON format
 * 
 */
public final class ECKeystore {
	 ...
	 * 
	 * @param os - stream to write JSON format to
	 * @param password Scrypt password used to derive AES key
	 * @param keys private keys to output
	 * @throws Exception
	 */
	public static void export(OutputStream os, String password,
			Collection<PrivateKey> keys) throws Exception {

		KeyCrypterScrypt crypterScrypt = new KeyCrypterScrypt(512);

		Protos.ScryptParameters params = crypterScrypt.getScryptParameters();
		ScryptHeader scryptHeader = new ScryptHeader.Builder().n(params.getN())
				.r(params.getR()).p(params.getP())
		...
		for (PrivateKey pk : keys) {
			...
			KeyEntry entry = new KeyEntry.Builder().alias(pk.nickname)
					.kty("EC").crv("P-256").pk(data.encryptedBytes)
	...
	/**
	 * Loads JSON keystore from the specified file and adds entries to the
	 * specified storage
	 * 
	 * @param file JSON keystore
	 * @param password Scrypt password used to derive AES key
	 * @param storage output for keystore to write to
	 * @throws Exception
	 */
	public static void load(File file, String password, Storage storage)
			throws Exception {
	 ...
	 * specified storage
	 * 
	 * @param is JSON keystore
	 * @param password Scrypt password used to derive AES key
	 * @param storage output for keystore to write to
	 * @throws Exception
	 */
	public static void load(InputStream is, String password, Storage storage)
			throws Exception {
		ObjectMapper mapper = new ObjectMapper();
		load(mapper.readValue(is, KeyStore.class), password, storage);
	}

	public static void load(KeyStore keystore, String password, Storage storage)
			throws Exception {
		Protos.ScryptParameters.Builder scryptParametersBuilder = Protos.ScryptParameters
				.newBuilder()
				.setSalt(
						ByteString.copyFrom(base64Url().decode(
								keystore.getScrypt().getSalt())))
		...
		for (KeyEntry entry : keystore.getKeys()) {
			...
			ECKey ecKey = ECKey
					.fromPrivate(crypterScrypt.decrypt(data, aesKey));

			String address = ecKey.toAddress(MainNetParams.get()).toString();
			
			storage.add(entry.getAlias(), address, ecKey.getPrivKeyBytes(),
					ecKey.getPubKey());
		}
	}
