...
public class DocumentArea2
	...
	public static class Inserter {
		...
		private TumblerAddress previousTumblerAddress;
		...
		public Optional<HyperOperation> createOperation(TumblerAddress tumblerAddress, int insertionEnd) {
			if (hasText()) {
				...
				int start = Math.max(0, insertionEnd - insertionText.length());
				HyperOperation op = new HyperOperation(tumblerAddress,
						new HyperRegion(start, insertionText.length()), OpCode.INSERT_TEXT,
		...
		public Optional<HyperOperation> flush() {	
			if(hasText()) {
				HyperOperation op = new HyperOperation(previousTumblerAddress,
						new HyperRegion(previousPosition - insertionText.length(), insertionText.length()), OpCode.INSERT_TEXT,
						insertionText.toString());
				return Optional.of(op);
		...
		public Optional<HyperOperation> insert(TumblerAddress tumblerAddress, PlainTextChange change) {
			...
			this.previousTumblerAddress = tumblerAddress;
	 ...
	 */
	public static DocumentArea2 newInstance(TumblerAddress tumblerAddress, BrowserContext ctx) {
		...
		return new DocumentArea2(tumblerAddress, ctx, styledTextOps._or(linkedImageOps),
	...
	private DocumentArea2(TumblerAddress homeDocument, BrowserContext ctx,
	...
	public void applyStyle(TumblerAddress span, TumblerAddress linkType) {
		StyleSpans<LinkType> styles = getStyleSpans(span.spanStart() - 1, span.spanStart() - 1 + span.spanWidth());
	...
	public void flush() {	
		Optional<HyperOperation> op = inserter.flush();
		if(op.isPresent()) {
	...
	public String opString() {
		...
		for (HyperOperation op : operations) {
			if (OpCode.INSERT_TEXT.equals(op.getOperation())) {
